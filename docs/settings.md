...
| `modelRoleFamilyRouting` | boolean | `false` | When enabled, selecting a default model applies session-only assignments for built-in roles. Claude uses Haiku for fast roles, Sonnet for routine roles, and Opus for planning/advisory roles when available. OpenAI Codex uses Luna, Terra/Sol, and Astra/Sol respectively. Local providers assign every built-in role to the selected local model, with no cloud role fallback. |
